...
export default class SkmService {
  ...
  public static async get_top_list() {
    const result = await http_get({ api: '/topList' });
    return result.data;
  }

  /** 获取列表 */
  public static async get_list(params: ListItemParamsType) {
    const result = await http_get({ api: '/list', params: { params } });
    return result.data;
  }

  /** 获取文件夹列表 */
  public static async get_folder_and_tag_list() {
    const result = await http_get({ api: '/folderAndTagList' });
    return result.data;
  }

  /** 获取文件夹或标签下内容 */
  public static async get_folder_or_tag_content(params: FolderOrTagContentParams) {
    const result = await http_get({ api: '/folderOrTagList', params: { params } });
    return result.data;
  }
}
